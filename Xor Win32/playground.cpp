...
playground::playground(QString ffile,QString p) {
    directory=p;
    setLevel(ffile);
    init();
}

playground::playground(QString ffile,QString p,Graphics *g) {
    directory=p;
    graph=g;
    setLevel(ffile);
    init();
...
void playground::showStatus(int wert) {
    if (wert==0 || wert==-1) {
		...
		// Der Spieler hat gewechselt
 		...
 		graph->RectRed(STATUS_X,175,80,30);
		QString schritte=toNum(anzahl);
    ...
    if (wert==1 || wert==-1) { // Maskenzaehler
		graph->Paint(&sprites[7],STATUS_X,220,0,0,80,80);
 		graph->RectRed(STATUS_X,265,80,30);
		QString masks=toNum(gesammelte_masken);
		QString masken=toNum(anzahl_masken);
		masks=masks+"/"+masken;
...
void playground::getLevelname() {
    ...
    //char a[200];
    //for (unsigned int ii=0;ii<level.length();ii++)
	//	a[ii] = level[ii];
    FILE *ffeld;
    ffeld=fopen(level,"r");
    if (!ffeld) {
	...
	levels[i]='\0';
	fclose(ffeld);
	level_name=(QString)levels;
    } 
    //cout << "LEVEL ENDE____" << endl;
}

void playground::setLevel(QString i) {
...
void playground::saveMoves(QString name) {
    ofstream file_out;
    int i=0;
...
void playground::loadMoves(QString name) {
    FILE *ffeld;
    char levels[200];
    //name = directory+name;
    // const char *na=name;
...
//	a[i]=name[i];
    ...
    setLevel((QString)levels);
    
    //cout << "LEVEL......" << level << endl;

    //level = (QString)levels;
    getLevelname();
    i=0;
    while (i<=MAX_STEP) {
	if (fscanf(ffeld,"%d",&k)!= EOF ) {
...
void playground::loader() { // Lade ein Spielfeld
	...
	//cout << names[i] << endl;
		sprites[ii].Load(names[ii]); 
    }
    QString name,name_l,name_r,temp;
    int ij;
    temp = names[12].left(names[12].find('.')); //substr(0,names[12].find('.'));
    for (ij=0;ij<8;ij++) {
	
	  name = temp + "_"+toNum(ij+1)+".bmp";
	name_l = temp + "_"+toNum(ij+1)+"_left.bmp";
	name_r = temp + "_"+toNum(ij+1)+"_right.bmp";
	bomben[ij][0].Load(name);
	bomben[ij][1].Load(name_l);
	bomben[ij][2].Load(name_r);
    }
    temp = names[13].left(names[13].find('.'));//substr(0,names[13].find('.'));
    for (i=0;i<8;i++) {
